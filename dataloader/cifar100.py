...
def get(seed=0,pc_valid=0.10):
    ...
    if not os.path.isdir(file_dir):
        ...
        dat['train_original'] = datasets.CIFAR100(cf100_dir, train=True, download=True, transform=transforms.Compose([transforms.ToTensor()]))
        ...
        for n in range(10):
            ...
            data[n]['train']={'x': [],'y': []}
            data[n]['test']={'x': [],'y': []}
            data[n]['train_original'] = {'x': [], 'y': []}
        for s in ['train','test', 'train_original']:
    ...
    for i in range(10):
        data[i] = dict.fromkeys(['name','ncla','train','test', 'train_original'])
        for s in ['train','test', 'train_original']:
            data[i][s]={'x':[],'y':[]}
    ...
    for t in data.keys():
        ...
        data[t]['train_original']['x'] = data[t]['train_original']['x'][itrain].clone()
        data[t]['train_original']['y'] = data[t]['train_original']['y'][itrain].clone()
