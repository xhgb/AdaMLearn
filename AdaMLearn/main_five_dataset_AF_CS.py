...
from deepcore.nets.resnet import ResNet_32x32, BasicBlock
...
def get_representation_matrix_ResNet18(net, device, x, y, dst_train, task_id):
    ...
    batch_list = np.concatenate((np.array([int(example_num/20)] * 8),
                                 np.array([int(example_num/5)] * 3),
                                 np.array([int(example_num)] * 6)))
    # network arch
    ...
    sc_list=[5,9,13]
    p1d = (1, 1, 1, 1)
    mat_final=[] # list containing memory Matrices
    mat_list=[]
    mat_sc_list=[]
...
def main(args):
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            model = ResNet_32x32(BasicBlock, [2, 2, 2, 2], num_features=args.num_features, backpack=args.backpack).to(device)
...
if __name__ == "__main__":
    # Training parameters
    parser = argparse.ArgumentParser(description='5 datasets')
    ...
    parser.add_argument('--active_forget', default=0.9999, type=float,
    ...
    parser.add_argument('--fraction', default=0.001, type=float, help='fraction of data to be selected (default: 0.01)')
    ...
    parser.add_argument('--model_name', type=str, default='ResNet_32x32', help='model')
