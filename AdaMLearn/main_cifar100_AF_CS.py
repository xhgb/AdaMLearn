...
from deepcore.nets import AlexNet_32x32_AdaMLearn
...
def get_representation_matrix (net, device, x, y, dst_train, task_id):
    ...
    net.features[10].register_forward_hook(get_activation('conv_3', act_dict))
    net.features_lin[0].register_forward_hook(get_activation('fc_1', act_dict))
    net.features_lin[4].register_forward_hook(get_activation('fc_2', act_dict))

    example_data = example_data.to(device)
    example_out = net(example_data)
    batch_list = [example_num, example_num, example_num, example_num, example_num]
    mat_list=[]
    ...
    for i in range(len(act_key)):
        ...
        if i<3:
            ksz= ksize[i]
            s=compute_conv_output_size(map[i],ksz)
...
def main(args):
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            model = AlexNet_32x32_AdaMLearn(channel=3, num_classes=100, hidden_channels=128, backpack=args.backpack).to(device)
...
if __name__ == "__main__":
    # Training parameters
    parser = argparse.ArgumentParser(description='Sequential CIFAR-100')
    ...
    parser.add_argument('--active_forget', default=0.99, type=float,
                        help='proportion of active_forget (default: 0.99)')
    ...
    parser.add_argument('--example_num', default=50, type=int, help='random selected samples (default: 50)')
    ...
    parser.add_argument('--model_name', type=str, default='AlexNet_32x32', help='model')
