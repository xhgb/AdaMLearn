...
class Submodular(EarlyTrain):
    ...
    def calc_gradient_and_hess(self, index=None):
        ...
        for i, (input, targets, *rest) in enumerate(batch_loader):
            self.model_optimizer.zero_grad()
            outputs = self.model(input.to(self.args.device))
    ...
    def directly_calc_gradient_and_hess(self, index=None, task_id=None):
        '''
        Calculate gradients matrix on current network for specified training dataset.
        '''
        self.criterion = extend(self.criterion)
        ...
        if task_id is not None:
            self.embedding_dim = self.model.get_last_layer(task_id).in_features
        else:
            self.embedding_dim = self.model.get_last_layer().in_features
        ...
        r = index
        np.random.shuffle(r)
        r = torch.LongTensor(r).to(self.args.device)
        # Loop batches
        for i in range(0, len(r), self.args.batch_size_train):
            if i + self.args.batch_size_train <= len(r):
                b = r[i:i + self.args.batch_size_train]
            else:
                b = r[i:]
            self.model_optimizer.zero_grad()
            if self.model_name == 'MLP':
                inputs = self.dst_train[b].view(-1, 28 * 28).to(self.args.device)
            else:
                inputs = self.dst_train[b].to(self.args.device)
            targets = self.dst_train.targets.to(self.args.device)[b]
            outputs = self.model(inputs.to(self.args.device))
            if task_id is not None:
                loss = self.criterion(outputs[task_id], targets.to(self.args.device))
            else:
                loss = self.criterion(outputs,targets.to(self.args.device))
            batch_num = targets.shape[0]
            ...
            for name, param in self.model.named_parameters():
                if task_id is not None:
                    if 'classifier.' + str(task_id) + '.weight' in name:
                        weight_parameters_grads = param.grad_batch
                        weight_parameters_hesses = param.diag_h_batch
                        # print(weight_parameters_hesses.shape)
                    elif 'classifier.' + str(task_id) + '.bias' in name:
                        bias_parameters_grads = param.grad_batch
                        bias_parameters_hesses = param.diag_h_batch
                        # print(bias_parameters_hesses.shape)
                else:
            ...
            gradients.append(weight_parameters_grads.flatten(1).cpu().numpy())
            hessians.append(weight_parameters_hesses.flatten(1).cpu().numpy())
    ...
    def directly_select(self, model, task_id=None):
        self.model = model
        ...
        with self.model.embedding_recorder:
            ...
            if self.balance:
                ...
                for c in range(self.num_classes):
                    ...
                    if self.args.kernel == 'worst':
                        gradients, hessians = self.directly_calc_gradient_and_hess(index=c_indx, task_id=task_id)
                        if self.args.exact_analyses:
                            self.value_dict['gradient_origin_' + str(c)] = gradients
                            self.value_dict['diag_hessian_origin_'+str(c)] = hessians
                            self.value_dict['index_origin_' + str(c)] = c_indx
                        ...
                        gradients, hessians = self.directly_calc_gradient_and_hess(index=c_indx)
