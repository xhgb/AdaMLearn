...
class ResNet_32x32(nn.Module):
    def __init__(self, block, num_blocks, num_features=64, channel=3, num_classes=10, record_embedding: bool = False,
                 no_grad: bool = False,backpack = False):
        super().__init__()
        self.in_planes = num_features
        self.conv1 = conv3x3(channel, num_features)
        self.bn1 = nn.BatchNorm2d(num_features, track_running_stats=False)
        ...
        if backpack:
            for i in range(5):
                self.classifier.append(extend(nn.Linear(num_features * 32 * block.expansion, num_classes, bias=False)))
        else:
            for i in range(5):
                self.classifier.append(nn.Linear(num_features * 32 * block.expansion, num_classes, bias=False))
    ...
    def forward(self, x):
        with set_grad_enabled(not self.no_grad):
            ...
            for i in range(5):
                y.append(self.classifier[i](out))
        return y
...
class ResNet_84x84(nn.Module):
    def __init__(self, block, num_blocks, num_features=64, channel=3, num_classes=5, record_embedding: bool = False,
                 no_grad: bool = False,backpack = False):
        super().__init__()
        self.in_planes = num_features
        self.conv1 = conv3x3(channel, num_features)
        ...
        if backpack:
            for i in range(20):
                self.classifier.append(extend(nn.Linear(num_features * 200 * block.expansion, num_classes, bias=False)))
        else:
            for i in range(20):
                self.classifier.append(nn.Linear(num_features * 200 * block.expansion, num_classes, bias=False))
